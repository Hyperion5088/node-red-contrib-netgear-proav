...
class NetgearProAvClient {
  ...
  async configStatus() {
    const result = {
      configSaveNeeded: null,
      runningConfigMatchesSaved: null,
      source: "unsupported_or_not_reported",
      deviceInfo: {},
      imageInfo: {}
    };

    try {
      result.deviceInfo = await this.deviceInfo();
    } catch (error) {
      result.deviceInfoError = error.message;
    }

    try {
      result.imageInfo = await this.imageInfo();
    } catch (error) {
      result.imageInfoError = error.message;
    }

    const status = configSaveNeededFromSources(result.deviceInfo, result.imageInfo);
    result.configSaveNeeded = status.value;
    result.runningConfigMatchesSaved = status.value === null ? null : !status.value;
  ...
  saveConfig() {
    return this.post("switch_config", undefined, undefined, { allowEmpty: true });
  }
...
function configSaveNeededFromSources(...sources) {
  for (const [sourceName, source] of [
    ["device_info", sources[0]],
    ["image_info", sources[1]]
  ]) {
    const value = configSaveNeededFromObject(source);
    if (value !== null) {
      return { value, source: sourceName };
    }
  }
  return { value: null, source: "unsupported_or_not_reported" };
}

function configSaveNeededFromObject(source) {
  if (!source || typeof source !== "object") {
    return null;
  }
  const nested = source.deviceInfo && typeof source.deviceInfo === "object" ? source.deviceInfo : source;
  for (const key of ["configSaveNeeded", "saveNeeded", "unsavedChanges", "runningConfigChanged"]) {
    const parsed = parseConfigSaveNeeded(nested[key]);
    if (parsed !== null) {
      return parsed;
    }
  }
...
function parseConfigSaveNeeded(value) {
  ...
  if (["true", "yes", "on", "1", "needed", "unsaved", "dirty"].includes(text)) {
    return true;
  }
  if (["false", "no", "off", "0", "saved", "clean"].includes(text)) {
    return false;
  }
  return null;
}
